...
public class AdminOrderServiceImpl implements AdminOrderService {
    @Autowired
    private RestTemplate persistance;

    @Override
    public  FindObjects findEveryObjectWithLoginId(String loginId) {
        FindObjects contacts ;
        if(idUser.equals(idUserName)){
            contacts = persistance.getForObject(
                    "http://ts-contacts-service:12347/contacts/findAll",
                    FindObjects.class);
        } else {
            contacts = new FindObjects();
            contacts.setStatus(false);
            contacts.setComment("The loginId is Wrong: " + loginId);
        }
